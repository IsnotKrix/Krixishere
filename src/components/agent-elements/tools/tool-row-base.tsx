...
export function ToolRowBase({
  ...
  const row = (
    <div
      className={cn(
        "flex items-center max-w-full select-none gap-1 rounded-lg",
    ...
    >
      <div className="flex items-center gap-2 min-w-0 text-sm text-zinc-500">
        ...
        {detail && (
          <span className="font-normal truncate min-w-0 flex-1 text-zinc-600">
            {detail}
          </span>
        )}
        {trailingContent}
      ...
      {expandable && (isComplete || isExpanded || isAnimating) && (
        <div>
          <IconChevronRight
            className={cn(
              "shrink-0 text-zinc-500 transition-transform duration-150 ease-out",
              "size-3",
