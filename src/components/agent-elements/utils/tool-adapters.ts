...
export function mapToolInvocationToStep(
  ...
  if (toolName === "Edit" && Array.isArray((result as Record<string, unknown>)?.structuredPatch)) {
    step.diffStats = calculateDiffStatsFromPatch((result as Record<string, unknown>).structuredPatch as Array<{ lines?: string[] }>);
    step.diffLines = getDiffLinesFromPatch((result as Record<string, unknown>).structuredPatch as Array<{ lines?: string[] }>);
  }
