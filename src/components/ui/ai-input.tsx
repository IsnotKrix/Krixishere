...
export interface MorphPanelProps {
  /** Called with the submitted message text when the user hits send */
  onSend?: (message: string) => void
}

export function MorphPanel({ onSend }: MorphPanelProps) {
      ...
      <AnimatePresence>
        {showForm && (
          ...
          >
            ...
            <textarea
              ref={ref}
              placeholder="Ask me anything about the changelog…"
              name="message"
              className="h-full w-full resize-none scroll-py-2 rounded-md p-4 bg-transparent text-foreground placeholder:text-muted-foreground outline-0 text-sm"
              required
