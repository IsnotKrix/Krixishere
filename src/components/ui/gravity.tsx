...
const Gravity = forwardRef<GravityRef, GravityProps>(
  (
    ...
    const initializeRenderer = useCallback(() => {
      ...
      import("poly-decomp").then((decomp) => {
      ...
      // Matter.js adds touch + wheel listeners with preventDefault() which blocks native
      // page scroll. Strip all of them — desktop drag still works via mouse events.
      const mc = render.current.canvas
      const m = mouse as unknown as Record<string, EventListener>
      mc.removeEventListener("mousewheel", m["mousewheel"])
      mc.removeEventListener("DOMMouseScroll", m["mousewheel"])
      mc.removeEventListener("touchstart", m["touchstart"])
      mc.removeEventListener("touchmove", m["touchmove"])
      mc.removeEventListener("touchend", m["touchend"])
      // Allow browser to scroll the page natively on touch devices.
      mc.style.touchAction = "auto"
